<script>
    ...
    function setSubmitAttributes(value){
        ...
        if($userStore.hobbies == "" || $userStore.superPower == ""){
            inputError=true;
        }
        else{
...
<main>
    ...
    <div class="rounded-content-box container p-3">
                ...
                <div>
                    <form action="" class="form-text">
                        ...
                        <input type="text" id="hobbies" class="form-control form-control-lg" class:input_error={inputError} 
                                placeholder={(inputError? (isFr? "Veuillez entrer un passe-temps" : "Please enter a hobby") : (isFr? "Ex: enginérie, piano, jeux video" : "e.g. engienneering,  play piano, video games"))} 
        ...
        {:else}
            <p class="description-text text-center">{(isFr ? `Il parait que vous êtes un maitre de ${$userStore.superPower} et que vouse aimez ${$userStore.hobbies}!` : `It looks like you are a master of ${$userStore.superPower} and you enjoy ${$userStore.hobbies}!`)}</p>
